...
import MyPostForm, {PostFormData} from "./MyPostForm/MyPostForm";
...
const MyPost: FC<PropsType> = React.memo((
    ...
    const postElements = postData.map((post: PostType) => (
        <Post
            key={post.id}
            ...
            login={login}
            post={post}
            incrementedLike={incrementedLike}
            deletePost={deletePost}
        />
    ));

    const onSubmit = (formData: PostFormData) => addPost(formData.post);
